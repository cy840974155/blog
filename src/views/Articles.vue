        ...
        <article class="hentry">
          ...
          <header class="entry-header">
            ...
            <div class="breadcrumbs">
              <div id="crumbs">
                最后更新时间：{{ article.release_time | parseTime }}&nbsp;作者：{{article.name}}
              </div>
            </div>
          </header>
          <!-- 正文输出 -->
...
yum -y install gcc-c++</code></pre>
                        ...
                        <hr> -->
            <!-- <h4 :id="article.desc">{{ article.desc }}</h4> -->
            <blockquote>
              <p>{{ article.content }}</p>
            </blockquote>
...
export default {
  ...
  methods: {
    ...
    getFetchArticle() {
      ...
      this.query = window.location.pathname.split("/")[2];
      fetchArticle({ id: this.query })
        .then((res) => {
